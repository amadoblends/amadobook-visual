    ...
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100dvh', background:'#F5F5F5', fontFamily:"'DM Sans', system-ui, sans-serif", maxWidth:430, margin:'0 auto' }}>
      <div style={{ background:'#FFF', paddingTop:'env(safe-area-inset-top,16px)', borderBottom:'1px solid #F0F0F0', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px' }}>
          ...
          {unread > 0 ? (
            <button onClick={markAll} style={{ background:'none', border:'none', cursor:'pointer', fontSize:12, color:'#FF6B1A', fontWeight:600, fontFamily:'inherit' }}>Leer todas</button>
          ) : <div style={{ width:60 }} />}
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:16, display:'flex', flexDirection:'column', gap:10 }}>
        {notifs.length === 0 ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', paddingTop:64, gap:14 }}>
            <div style={{ width:72, height:72, borderRadius:'50%', background:'#F5F5F5', display:'flex', alignItems:'center', justifyContent:'center' }}><Bell size={32} color="#CCC" /></div>
            <p style={{ fontSize:17, fontWeight:700, color:'#0D0D0D', margin:0 }}>Sin notificaciones</p>
            <p style={{ fontSize:14, color:'#999', textAlign:'center', maxWidth:240, lineHeight:1.6, margin:0 }}>Aquí aparecerán tus recordatorios y confirmaciones.</p>
          </div>
        ) : notifs.map((n:any) => {
          const cfg = ICONS[n.type] ?? { bg:'#F5F5F5', icon:'🔔' }
          const diff = Date.now() - new Date(n.sent_at).getTime()
          const mins = Math.floor(diff/60000)
          const time = mins < 60 ? `${mins} min` : mins < 1440 ? `${Math.floor(mins/60)} h` : 'Ayer'
          return (
            <div key={n.id} style={{ display:'flex', alignItems:'flex-start', gap:14, background: n.is_read?'#FFF':'#FFFAF7', borderRadius:14, padding:'14px 16px', border: n.is_read?'1px solid #F0F0F0':'1px solid #FFD4B8' }}>
              <div style={{ width:44, height:44, borderRadius:12, background:cfg.bg, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>{cfg.icon}</div>
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontSize:14, fontWeight: n.is_read?500:700, color:'#0D0D0D', margin:'0 0 4px', lineHeight:1.4 }}>{n.title}</p>
                <p style={{ fontSize:13, color:'#666', lineHeight:1.5, margin:'0 0 6px' }}>{n.body}</p>
                <p style={{ fontSize:11, color:'#CCC', margin:0 }}>{time}</p>
              </div>
              {!n.is_read && <div style={{ width:8, height:8, borderRadius:'50%', background:'#FF6B1A', flexShrink:0, marginTop:6 }} />}
            </div>
          )
        })}
        <div style={{ height:40 }} />
