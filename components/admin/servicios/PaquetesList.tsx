// AMADOBOOK — PaquetesList.tsx
// Lista de paquetes con buscador, stats y botón crear paquete
// Ruta: /admin/servicios (tab Paquetes)

'use client'

import { useState, useMemo } from 'react'
import PaqueteCard, { PaqueteCardProps } from './PaqueteCard'

export interface PaquetesListProps {
  packages: PaqueteCardProps[]
  totalPackages: number
  activePackages: number
  packagesWithOffer: number
...
export default function PaquetesList({
  packages, totalPackages, activePackages, packagesWithOffer,
  activeTab, onTabChange,
  onNewPaquete, onEdit, onToggleActive, onDelete,
}: PaquetesListProps) {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#0D0D0D' }}>Paquetes</div>
              <div style={{ fontSize: 12, color: '#999' }}>Crea combinaciones de servicios</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, padding: '0 16px 12px' }}>
          {[
            { label: 'Paquetes', value: totalPackages,    icon: '✂️', color: '#0D0D0D' },
            { label: 'Activos',  value: activePackages,   icon: '✓',  color: '#16A34A' },
            { label: 'Con oferta',value: packagesWithOffer,icon: '🏷️', color: '#FF6B1A' },
          ].map((s, i) => (
            <div key={i} style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {filtered.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 60, gap: 12 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: '#0D0D0D' }}>Sin paquetes</div>
